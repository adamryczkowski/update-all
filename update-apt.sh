...
function find_apt_list {
	...
	if [ -n "$(find /etc/apt/sources.list.d -name '*.list' | head -1)" ]; then
		# List files and search for phrase
		grep -l -e "${phrase}" /etc/apt/sources.list.d/*.list
	fi
}
...
function turn_https_all {
	find_apt_list "$1" | while IFS= read -r file; do turn_https "${file}"; done
}

function turn_http_all {
	find_apt_list "$1" | while IFS= read -r file; do turn_http "${file}"; done
}
	...
	if is_host_tcp_port_up "$aptproxy_ip" "$aptproxy_port" >/dev/null; then
   	echo "aptproxy ${aptproxy_ip}:${aptproxy_port} seems up and running!"
		...
		if [ -n "$aptproxy_enabled" ]; then
			echo "Acquire::http::Proxy \"http://${aptproxy_ip}:${aptproxy_port}\";" | sudo tee "${aptproxy_file}" >/dev/null
		fi
	else
